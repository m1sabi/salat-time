...
// latitude rule for calc methods, and dynamic hiding of UOIF below 45°
// ============================
...
function triggerAzan(prayerKey) {
  if (Date.now() < cooldownUntil) return;

  const now = new Date();
  const minuteKey = `${String(now.getHours()).padStart(2,"0")}:${String(now.getMinutes()).padStart(2,"0")}`;
  const cap = {
    sunrise:"Sunrise", fajr:"Fajr", dhuhr:"Dhuhr", asr:"Asr",
    maghrib:"Maghrib", ishaa:"Ishaa",
    makkah_maghrib:"Maghrib_Makkah",
    makkah_ishaa:"Ishaa_Makkah"
  }[prayerKey];
  if (!cap) return;
  if (triggeredThisMinute[cap] === minuteKey) return;
  triggeredThisMinute[cap] = minuteKey;

  if (alarmEnabled[prayerKey]) {
    elements.azanSound.play().catch(() => {});
  }
...
function checkAzanTrigger() {
  ...
  const map = [
    { key: "sunrise", txt: elements.sunriseCell.textContent },
    { key: "fajr",    txt: elements.fajrCell.textContent },
    { key: "dhuhr",   txt: elements.dhuhrCell?.textContent || "" },
    { key: "asr",     txt: elements.asrCell?.textContent   || "" },
    { key: "maghrib", txt: elements.sunsetCell.textContent },
    { key: "ishaa",   txt: elements.ishaaCell.textContent },
    { key: "makkah_maghrib", txt: elements.makkahFotorCell?.textContent?.slice(0,5) || "" },
    { key: "makkah_ishaa",   txt: elements.makkahIshaaCell?.textContent?.slice(0,5) || "" }
  ];

  for (const { key, txt } of map) {
    if (txt && txt.includes(":") && txt === cur) {
      triggerAzan(key);
    }
  }
}
...
window.onload = () => {
  ...
  setInterval(() => { checkAzanTrigger(); }, 5000);
};
