...
// single-fire Azan scheduler + SW notification "Stop Azan" action.
// ============================

// Try to ensure our SW is available alongside OneSignal's SW (different scope)
if ('serviceWorker' in navigator) {
  // Safe to register — OneSignal is scoped to /salat-time/, ours is default scope.
...
// ---- Mobile audio unlock (add to main.js once) ----
let __audioUnlocked = false;
function unlockAzanAudioOnce() {
  if (__audioUnlocked) return;
  const el = document.getElementById('azanSound');
  if (!el) return;
  // Try to start & immediately pause — this "unlocks" the media
  el.play().then(() => { try { el.pause(); el.currentTime = 0; } catch(_) {} __audioUnlocked = true; })
           .catch(() => { /* ignore; we’ll try again on the next gesture */ });
}
// Call on the first user gesture:
window.addEventListener('pointerdown', unlockAzanAudioOnce, { once: true });
// Also tie to your existing Preview button just in case:
...
async function updateTable() {
  ...
  // NEW: feed the single-fire scheduler with today's times
  scheduleAzanFromCells();

  // countdown + clock color
  checkAndStartCountdown();

  if (usedUrl) console.info(`[salat] Loaded ${cityKey} from: ${usedUrl}`);
}
...
// ====== SINGLE-FIRE AZAN SCHEDULER (new) ======
...
if (navigator.serviceWorker?.controller) {
  navigator.serviceWorker.ready.then(reg => {
	reg.showNotification(`Azan – ${prayerId}`, {
	  body: `It's time for ${prayerId}.`,
	  vibrate: [200, 100, 200],
    ...
    if (Notification.permission === 'default') {
      Notification.requestPermission().then(() => { if (Notification.permission === 'granted') doShow(); });
    } else if (Notification.permission === 'granted') {
      doShow();
    }
  }
  ...
  return {
    updateTimes(obj) {
      times = { ...times, ...obj };
      // reset cooldown if day changed automatically thanks to new Date objects
    },
    tick() {
      const now = new Date();
...
// Legacy popup usage maintained (called when we also show SW notif)
...
window.onload = () => {
  ...
  // CLOCK: ensure we only start once
  ...
  // SCHEDULER LOOP: once per second for precise triggering
  ...
  // (Old per-5s trigger loop removed to avoid repeats)
};
