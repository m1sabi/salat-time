...
function timeToMinutes(t) {
  if (!t) return NaN;
  // Keep only digits and ":" (Android/RTL sometimes injects invisible marks)
...
function populateDayDropdown() {
  if (!elements.daySelect || elements.daySelect.options.length > 0) return;
  for (let i = 1; i <= 31; i++) {
    const opt = document.createElement("option");
...
function populateSoundOptions() {
  if (!elements.soundSelect || elements.soundSelect.options.length > 0) return;
  if (!window.soundOptions || !Array.isArray(window.soundOptions)) return;
...
function restoreSelectors() {
  ...
  if (savedSound) {
    ...
    elements.soundSelect.value = "trabulsi.mp3";
  }
...
function attachSelectorEvents() {
  ...
  elements.prevDayBtn?.addEventListener("click", () => {
    ...
    if (d > 1) {
      elements.daySelect.value = String(d - 1);
    } else {
      elements.daySelect.value = String(maxDay);
    }
    updateTable();
  });
...
function initializeApp() {
  initElements();
  if (typeof setAudioElement === 'function') setAudioElement(elements.azanSound);
  ...
  if (elements.countrySelect && elements.countrySelect.options.length === 0) {
    populateCountryDropdown();
  }
  if (elements.citySelect && elements.citySelect.options.length === 0) {
    rebuildCityDropdown();
  }
  if (elements.daySelect && elements.daySelect.options.length === 0) {
    populateDayDropdown();
  }
  if (elements.soundSelect && elements.soundSelect.options.length === 0) {
    populateSoundOptions();
  }
...
function setMakkahCell(cell, key, timeStr) {
  ...
  // If we intentionally hide the Makkah-based value, show a small help link in Maghrib row
  // instead of a dash, so users understand why.
  if (timeStr === "—" && key === "makkah_maghrib") {
    cell.innerHTML = `<a href="https://m1sabi.github.io/salat-time/about-makka.html"
      target="_blank" rel="noopener"
      style="color:#2e195f; text-decoration:none; font-weight:700;">
      حول توقيت مكة المكرمة
    </a>`;
    return;
  }
...
async function updateTable() {
  ...
  const isHighLatGate = typeof latGate === "number" && latGate >= 45;
  if (isHighLatGate) effectiveMethod = "UOIF";
  ...
  elements.sunsetCell.style.fontWeight = "700";
  ...
  const isHighLat = typeof lat === "number" && lat >= 49;
  const isLowLat  = typeof lat === "number" && lat < 49;

  elements.sunsetCell.style.color = isLowLat ? "#1D11C1" : "#2e195f";
  elements.ishaaCell.style.color = isLowLat ? "#1D11C1" : "";

  if (isHighLat) {
    elements.dhuhrCell.style.color = "#1D11C1";
    elements.asrCell.style.color = "#000";  // Keep Asr cell black for high latitudes
  } else {
    elements.dhuhrCell.style.color = "";
    elements.asrCell.style.color = "";
  }

  elements.makkahFotorCell.classList.toggle('blue', isHighLat);
  elements.makkahIshaaCell.classList.toggle('blue', isHighLat);

  document.dispatchEvent(new Event('latitude:updated'));
  applyCalcMethodUI();
  // --- Makkah-based fasting length (show only if THIS city's day/fast is longer than Makkah) ---
  // Idea:
  // 1) Compute this city's Fajr and Sunset (already in cells)
  // 2) Compute Makkah Fajr and Sunset for the same date
  // 3) If city fasting length > Makkah fasting length -> show the Makkah-based Maghrib/Ishaa column
  //    Otherwise show "—"
  ...
  // city fasting length (Sunset - Fajr)
  const cityFajrT = elements.fajrCell.textContent;
  const citySunsetT = elements.sunsetCell.textContent;

  const cityFajrMin0 = timeToMinutes(cityFajrT);
  const citySunsetMin0 = timeToMinutes(citySunsetT);
  let cityFastMin = citySunsetMin0 - cityFajrMin0;
  if (!Number.isFinite(cityFastMin)) {
    // If we can't parse city times reliably, don't show the Makkah column
    setMakkahCell(elements.makkahFotorCell, "makkah_maghrib", "—");
    setMakkahCell(elements.makkahIshaaCell, "makkah_ishaa",   "—");
  } else {
    if (cityFastMin < 0) cityFastMin += 1440;

  if (mSunrise && mSunset) {
    const mFajr   = subtract1h05(mSunrise);
    let makkahFastMin = timeToMinutes(mSunset) - timeToMinutes(mFajr);
    if (makkahFastMin < 0) makkahFastMin += 1440;

    if (cityFastMin > makkahFastMin && typeof lat === "number" && lat >= 49) {
      // Makkah-based Maghrib = city Fajr + (Makkah fasting length)
      let cityMagMin = cityFajrMin0 + makkahFastMin;
    ...
    // If we don't have Makkah times for this date (from the dataset), hide the column
    setMakkahCell(elements.makkahFotorCell, "makkah_maghrib", "—");
    setMakkahCell(elements.makkahIshaaCell, "makkah_ishaa",   "—");
  }
...
const AzanScheduler = (function () {
  ...
  let audio = null; // will be wired once elements are initialized
  let isPlaying = false;
...
function scheduleAzanFromCells() {
  ...
  // Get city latitude to determine if we should trigger Maghrib/Ishaa
  const cityKey = (elements.citySelect?.value || "").toLowerCase();
  const coords = getCityLatLng(cityKey);
  const lat = coords ? coords.lat : null;
  const isHighLat = typeof lat === "number" && lat >= 49;
  
  // Only set Maghrib and Ishaa times if latitude >= 49
  const maghrib = isHighLat ? parseTodayTimeToDate(elements.sunsetCell.textContent) : null;
  const ishaa   = isHighLat ? parseTodayTimeToDate(elements.ishaaCell.textContent) : null;
  ...
  if (typeof AzanScheduler !== 'undefined' && AzanScheduler && typeof AzanScheduler.updateTimes === 'function') {
