...
// ---- Mobile audio unlock (Edge/Chrome/Firefox Android friendly) ----
// Browsers block any audio until the user does ONE real gesture (tap/click).
// We "unlock" once per session by:
// - resuming AudioContext (if available)
// - playing the azan <audio> element very briefly (play -> pause -> reset)
// After that, scheduled Azan plays work while the page is open.
...
function unlockAzanAudioOnce() {
  ...
  // Ensure src is set BEFORE play (otherwise some browsers refuse)
  if (!el.src || el.src.endsWith("/azan/")) {
    const selected = document.getElementById("soundSelect")?.value || "hasan-bl3lol.mp3";
  ...
  // Resume AudioContext (helps on Android browsers)
  const ctx = ensureAudioContext();
  if (ctx && ctx.state === "suspended") {
    try { ctx.resume(); } catch (_) {}
  }

  // Try to play-pause quickly to unlock media pipeline
  el.play().then(() => {
    try {
      el.pause();
      el.currentTime = 0;
    ...
    // Still blocked => show banner (user must tap the button)
    showSoundUnlockBanner();
  });
}

// call this when Azan time arrives so we can show a banner if blocked
function tryPlayAzanNow() {
  const el = document.getElementById("azanSound");
  if (!el) return Promise.resolve(false);

  // If not unlocked yet, try once (might succeed if user already tapped)
  if (!__audioUnlocked) unlockAzanAudioOnce();

  return el.play().then(() => true).catch(() => {
    showSoundUnlockBanner("Tap to play Azan");
    return false;
  });
}

// Listen to many gesture types so unlock happens as early as possible.
// We keep them for the whole session; unlockAzanAudioOnce returns immediately after success.
["touchstart", "touchend", "pointerdown", "click", "keydown"].forEach((evt) => {
  window.addEventListener(evt, unlockAzanAudioOnce, { passive: true });
});

// Common UI interactions also unlock immediately
document.getElementById("previewBtn")?.addEventListener("click", unlockAzanAudioOnce);
document.getElementById("soundSelect")?.addEventListener("change", unlockAzanAudioOnce);
document.getElementById("countrySelect")?.addEventListener("change", unlockAzanAudioOnce);
document.getElementById("citySelect")?.addEventListener("change", unlockAzanAudioOnce);
document.getElementById("methodSelect")?.addEventListener("change", unlockAzanAudioOnce);
document.getElementById("daySelect")?.addEventListener("change", unlockAzanAudioOnce);
...
function setMakkahCell(cell, key, timeStr) {
  ...
  if (timeStr === "—" && key === "makkah_maghrib") {
    cell.innerHTML = `<a href="https://m1sabi.github.io/salat-time/about-makka.html"
      ...
      style="color:#32cd32; text-decoration:none; font-weight:700;">
...
const AzanScheduler = (function () {
  ...
  async function trigger(label, iconPath) {
    ...
    try { await tryPlayAzanNow(); } catch (e) { /* autoplay blocked, ignore */ }
  }
