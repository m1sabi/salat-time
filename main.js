...
// Compute timezone offset hours for a given IANA timezone at a specific date (including DST)
function tzOffsetHoursForDate(timeZone, dateObj) {
  try {
    if (!timeZone) return null;
    // Build the same "wall clock" time in the target tz and as UTC, then compare.
    ...
    // Timestamp representing that wall time interpreted in the given tz
    const asTz = Date.UTC(y, m-1, d, hh, mm, ss);
    // Same local wall clock interpreted as UTC
    const asUtc = Date.UTC(dateObj.getUTCFullYear(), dateObj.getUTCMonth(), dateObj.getUTCDate(),
                           dateObj.getUTCHours(), dateObj.getUTCMinutes(), dateObj.getUTCSeconds())
    // Offset minutes = (tz time - utc time) / 60000
    const offsetMin = Math.round((asTz - asUtc) / 60000);
    return offsetMin / 60;
  } catch (_) {
    return null;
  }
...
// ---- Mobile audio unlock (once per session) ----
...
window.addEventListener('pointerdown', unlockAzanAudioOnce, { once: true });
...
function getPTTimes(cityKey, dateISO, methodCode="MWL", asrSchool="Standard") {
  ...
  const tzName = getCityTimezone(cityKey);
  const tzHours = tzOffsetHoursForDate(tzName, localDate);
let baseMethod = "MWL";
  ...
  const times = pt.getTimes(localDate, [coords.lat, coords.lng], (typeof tzHours==='number'? tzHours : 'auto'), 0);
...
/* push scheduling moved below after Dhuhr/Asr are computed */


  // Push notifications (if backend is configured)
  try {
    const dateISO = todayISO;
    const dhuhr = (elements.dhuhrCell && elements.dhuhrCell.textContent) || '—';
    const asr   = (elements.asrCell   && elements.asrCell.textContent)   || '—';
    if (typeof scheduleTodaysPushes === 'function') {
      scheduleTodaysPushes({ dateISO, fajr, sunrise, dhuhr, asr, maghrib: sunset, ishaa }, getCityTimezone(elements.citySelect.value), document.querySelector('#citySelect option:checked')?.textContent || '');
    }
  } catch(_) {}
